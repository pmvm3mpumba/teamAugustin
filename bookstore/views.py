...
def uabook_form(request):
	return render(request, 'member/add_book.html')


@login_required
def request_form(request):
	return render(request, 'member/delete_request.html')


@login_required
def feedback_form(request):
	return render(request, 'member/send_feedback.html')
...
def usearch(request):
	...
	if len(data) == 0:
		...
		return render(request, 'member/result.html', {'files': files, 'word': word})
...
class UCreateChat(LoginRequiredMixin, CreateView):
	form_class = ChatForm
	model = Chat
	template_name = 'member/chat_form.html'
...
class UListChat(LoginRequiredMixin, ListView):
	model = Chat
	template_name = 'member/chat_list.html'
...
def create_user_form(request):
    choice = ['1', '0', 'Member', 'Admin', 'Librarian']
...
def asearch(request):
    ...
    if len(data) == 0:
        ...
        # word variable will be shown in html when user clicks on search button
        word = "Searched Result :"
        print("Result")
        print(res)
